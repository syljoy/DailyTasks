...
user = os.environ["STEP_USERS"]
passwd = os.environ["STEP_PASSWD"]
step = os.environ["STEP_NUMBER"]
...
def login(user_name, pwd):
    ...
    :return:
    """
    url1 = "https://api-user.huami.com/registrations/+86" + user_name + "/tokens"
    headers = {
    ...
    try:
        ...
        return 0, 0
    # print("access_code获取成功！code:", code)
    ...
    # print("login_token获取成功！login_token:", login_token)
    userid = r2["token_info"]["user_id"]
    # print("userid获取成功！userid:", userid)

    return login_token, userid


# 主函数
def main(user_name, pwd, step_num):
    ...
    if step_num == '':
        step_num = str(random.randint(18000, 26699))

    login_token, userid = login(user_name, pwd)
    if login_token == 0:
        print("登陆失败！")
        return "login fail!"
    ...
    # print(response)
    result = f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}] " \
             f"{user_name[:3]}****{user_name[-4:]}: 修改步数（{step_num}）- " \
             f"{response['message']}"
    return result
...
# token是服务端生成的一个字符串，以作客户端进行请求的一个令牌。当第一次登陆后，服务器生成一个
# token便将此token返回个客户端，以后客户端只需带上这个token前来请求数据即可，
# 无需再次带上用户名和密码。
...
def get_app_token(login_token):
    ...
    :return: app token
    """
    url = f"https://account-cn.huami.com/v1/client/app_tokens?app_name=com" \
          f".xiaomi.hm.health&dn=api-user.huami.com%2Capi-mifit.huami.com" \
          f"%2Capp-analytics.huami.com&login_token={login_token} "
    response = requests.get(url, headers=headers).json()
    app_token = response['token_info']['app_token']
    # print("app_token获取成功！app_token:", app_token)
    return app_token
...
if len(user_list) == len(passwd_list):
    for line in range(0, len(user_list)):
        if len(step_array) == 2:
            step = str(
                random.randint(int(step_array[0]), int(step_array[1])))
        elif str(step) == '0':
            step = ''
        result = main(user_list[line], passwd_list[line], step)
        print(result)
        time.sleep(10)
else:
    print('用户名和密码数量不对')
